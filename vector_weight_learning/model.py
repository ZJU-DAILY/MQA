...
class AggregationFunctionLoss(nn.Module):
    ...
    def forward(self, phi_positive, phi_negative):
        ...
        #     [torch.sum(-torch.log(phi_positive[i] / (phi_positive[i] + phi_negative[i])) for i in
        #      range(len(phi_positive)))])
        # for i in range(len(phi_positive)):
        #     print(phi_positive[i])

        loss = (1 / len(phi_positive)) * sum([torch.sum(-torch.log(
            torch.exp(phi_positive[i] / t) / (torch.exp(phi_positive[i] / t) + torch.exp(phi_negative[i] / t)))) for i
                                              in range(len(phi_positive))])
